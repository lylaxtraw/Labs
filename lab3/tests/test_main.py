...
def test_count_writes_bits_and_increments_when_up_is_pressed(monkeypatch):
	...
	assert [led.values for led in leds] == [[0], [0], [1], [0]]
	assert config.counter == 4


def test_count_stays_at_zero_when_down_is_pressed_at_lower_limit(monkeypatch):
...
def test_count_stays_at_100_when_up_is_pressed_at_upper_limit(monkeypatch):
	library, config, leds = load_library(monkeypatch, 100, 0, 1)

	with pytest.raises(StopLoop):
		library.count()

	assert [led.values for led in leds] == [[0], [0], [1], [0]]
	assert config.counter == 100
